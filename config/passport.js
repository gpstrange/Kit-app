...
module.exports = function(passport) {
  ...
  passport.deserializeUser(function(user, done) {
    if (user.role) {
      ...
      connection.query("SELECT * FROM users WHERE id = ? ", [user.id], function(
        err,
        rows
      ) {
        done(err, rows[0]);
        ...
        connection.query(
          ...
          function(err, rows) {
            ...
            if (rows.length) {
              ...
              var insertQuery =
                "INSERT INTO staffs ( username, password, address, name, email, dept, dob, phone, pic, community, bloodGroup, aadharNumber ) values (?,?,?,?,?,?,?,?,?,?,?,?)";
